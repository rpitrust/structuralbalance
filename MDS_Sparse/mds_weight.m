function [X,hist] = mds_weight(file_name, dim, path)

graph_data=dlmread(file_name, '\t');

graph_data(:,1:2)=graph_data(:,1:2)+1;
graph_data(:,3)=graph_data(:,3)+3;
...
% start optimization
[X,hist] = smacof_weight(Adj,X0,iter,verbose,xhistory,rtol,atol, tol, wp, wn, wu, dp, dn, du, wwp, wwn, dwp, dwn);
